...
paircomp <- function(y,x1,g,data,phm="holm",nonparm=FALSE){
  ...
  # one-sample
  cohen_d_onesample <- function(x, mu = 0){
    x <- x[!is.na(x)]
    (mean(x) - mu) / sd(x)
  }
...
ui =
  fluidPage(
    ...
    titlePanel("体育学位论文数据在线分析平台",title=div("体育学位论文数据在线分析平台",style="text-align: center; color: #00aed1;")),
    #br(),
    #br(),
    br(),
    ...
    sidebarPanel(
      ...
      conditionalPanel(#condition = "$('li.active a').first().html()==='About T-test'",
        ...
        div(img(src='Logo.png',height="60%",width="60%"),style="text-align: center;"), #,align="center")
        helpText("体育学位论文数据在线分析平台",align="center",style="color: #000000;line-height: 1.8;font-size: 22px;"),
        helpText("由淮北师范大学体育学院孟栗典先生和郑贺教授开发",align="center",style="color: #000000;line-height: 1.8;font-size: 17px;"),
        helpText("引用：Meng L, Zheng H. “OPSTA: An Online Analysis Platform for Chinese Sports Science Thesis Data Based on the Shiny Framework.” bioRxiv, 26 Dec. 2025, doi:10.64898/2025.12.26.696552.",align="center",style="color: #000000;line-height: 1.8;font-size: 17px;")
        ),
      ...
      conditionalPanel(
        ...
        div(
          ...
          h4("使用体育学位论文数据在线分析平台，可在文章中如下描述："),
          
          tags$ul(
            tags$li("比如在数理统计法部分：使用基于R 4.5.1开发的数据在线分析平台（https://menglab.org.cn/opsta/）进行数据分析。"),
            tags$li("比如在致谢部分：感谢淮北师范大学体育学院的孟栗典先生和郑贺教授，提供了关于数据分析（https://menglab.org.cn/opsta/)的建议。"),
          )
        )
      ),
      ...
      conditionalPanel(#condition = "$('li.active a').first().html()==='Data View'",
        ...
        fileInput('file1', '导入CSV文件',
                  accept=c('text/csv', 
                           'text/comma-separated-values,text/plain', 
                           '.csv')),
        
        checkboxInput('header', '表头', TRUE),
        ...
        tags$hr(),
        textInput("rname.data", "选择 R 数据（输入：包名::数据名，例如：survival::aml)", ""),
        tags$hr()
        #,
        #radioButtons("warn",
      ...
      conditionalPanel(#condition = "$('li.active a').first().html()==='Data View'",
        ...
        helpText("注意：仅适用于数值型变量")
        
      ),
      
      # 信度和效度分析
      conditionalPanel(
        'input.method === "信度和效度分析"',
        
        h4("信度分析"),
        selectInput("reli_items",
                    label = "请选择量表条目（可多选）：",
                    choices = NULL,
                    multiple = TRUE),
        
        h4("效度分析"),
        selectInput("validity_var",
                    label = "请选择效标变量（可选）：",
                    choices = NULL),
        
        radioButtons("validity_type",
                     "效度分析方式：",
                     choices = c("总分–效标相关" = "corr",
                                 "暂不进行效度分析" = "none"),
                     selected = "corr"),
        
        helpText("提示：信度推荐至少选择 3 个条目；效度分析需要至少选择一个效标变量。")
      ),
      ...
      conditionalPanel(#condition = "$('li.active a').first().html()==='T-test'",
        ...
        numericInput("conf.ttest",
                     ...
                     min = 0.8,
                     max = 0.99),
        helpText("注意：请输入一个0到1之间的数值")
        
      ),
      
      # SliderbarPanel for one-way ANOVA
      conditionalPanel(#condition = "$('li.active a').first().html()==='About T-test'",
        'input.method === "单因素方差分析"',
        ...
        radioButtons("posthoc.anova",
                     "请选择两两事后比较（post-hoc）P值的校正方法：",
                     choices = c("Holm 校正" = "holm",
                                 "BBonferroni 校正" = "bonferroni", 
                                 "Benjamini–Hochberg（BH / FDR）校正" = "BH",
                                 "不进行校正" = "none")
        )
        #,
      ...
      conditionalPanel(#condition = "$('li.active a').first().html()==='About T-test'",
        'input.method === "双因素方差分析"',
        selectInput("var1_tanova", 
                    label = "请选择结局变量（Y）",
                    ""
        ),
        radioButtons("transform.tanova",
                     "请选择仅用于假设检验的 Y 变量变换类型：",
                     choices = c("不进行变换" = "none",
                                 "对数变换" = "logtrans", 
                                 "平方根变换" = "srtrans",
                                 "秩变换（用于非参数分析）" = "ranktrans")
        ),
        ...
        textInput("xlab.tanova", "X 轴标签（X）："),
        textInput("ylab.tanova", "Y 轴标签（Y）："),
        textInput("tlab.tanova", "图例标签（G）："),
        numericInput("pdodge.tanova", label = "组间箱线图间距：", value = 1),
        
        radioButtons("posthoc.tanova",
                     "请选择两两事后比较（post-hoc）P 值的校正方法：",
                     choices = c("Holm 校正" = "holm",
                                 "Bonferroni 校正" = "bonferroni", 
                                 "Benjamini–Hochberg（BH / FDR）校正" = "BH",
                                 "不进行校正" = "none")
        )
        #,
      ...
      conditionalPanel(#condition = "$('li.active a').first().html()==='About T-test'",
        ...
        selectInput("indepcat_reg", 
                    label = "请在已选择的协变量中，标明哪些是分类协变量",
                    "",
                    multiple=TRUE
        ),
        selectInput("scale.reg", 
                    label = "请选择需要进行标准化的连续型协变量",
                    "",
                    multiple=TRUE
        )
      ...
      conditionalPanel(#condition = "$('li.active a').first().html()==='About T-test'",
        ...
        selectInput("log.cor", 
                    label = "请选择需要进行对数变换（log）的数值型变量",
                    "",
                    multiple=TRUE
        ),
        selectInput("sqr.cor", 
                    label = "请选择需要进行平方根变换（square root）的数值型变量",
                    "",
                    multiple=TRUE
        ),
        radioButtons("cmet_cor",
                     "请选择一种相关性分析类型：",
                     choices = c("Pearson 相关" = "pearson",
      ...
      conditionalPanel(#condition = "$('li.active a').first().html()==='T-test'",
        ...
        checkboxInput('header.ctab', '无表头', FALSE),
        textAreaInput("data.ctab", "数据：", "39 27\n66 153"),
      ...
      conditionalPanel(#condition = "$('li.active a').first().html()==='T-test'",
        ...
        radioButtons("BH_mcorrect",
                     "Benjamini–Hochberg 方法（1995；BH；又称 FDR，假发现率控制）",
                     choices = c("No" = "no",
                                 "是" = "yes")),
        radioButtons("BY_mcorrect",
                     "Benjamini–Yekutieli 方法（2001；BY）",
                     choices = c("否" = "no",
                                 "是" = "yes")),
        textAreaInput("data.mcorrect", "输入P值：", "0.01 0.5 0.7 0.03\n0.023 0.0001")
      )
    ...
    mainPanel(
        ...
        tabPanel('1. 关于',
                 ...
                 h4("欢迎使用体育学位论文数据在线分析平台，本平台基于 R 语言开发。告别统计软件的繁琐操作，快速完成体育领域的各类数据分析。本项目旨在解决中国体育学位论文中的的统计方法问题，确保结果具有科学性。服务器不会保存任何用户上传的数据文件及分析结果！",style="color: #194b78;line-height: 1.8;font-size: 20px;"),
                
                 tags$ul(   
                   tags$li("列联表分析，包括卡方检验、Fisher 精确检验、McNemar 检验和趋势检验。",style="color: #00aed1;line-height: 1.8;font-size: 18px;"),
                   tags$li("多重比较校正的 p 值调整方法，包括 Holm 校正、Bonferroni 校正、Hommel 校正、Hochberg 校正、Benjamini–Hochberg 校正和 Benjamini–Yekutieli 校正。",style="color: #006aa2;line-height: 1.8;font-size: 18px;"),
                   tags$li("数据集的数值和图形化描述性统计分析。",style="color: #00aed1;line-height: 1.8;font-size: 18px;"),
                   tags$li("单样本、双样本及配对样本的参数与非参数检验，包括 t 检验和 Wilcoxon 秩和检验。",style="color: #006aa2;line-height: 1.8;font-size: 18px;"),
                   tags$li("单因素和双因素方差分析（One-Way 与 Two-Way ANOVA。）",style="color: #00aed1;line-height: 1.8;font-size: 18px;"), 
                   #tags$li("肿瘤生长分析。",style="color: #006aa2;"),
                   tags$li("分类数据检验，包括卡方检验、Fisher 精确检验、McNemar 检验和趋势检验。",style="color: #006aa2;line-height: 1.8;font-size: 18px;"), 
                   tags$li("变量之间的相关性计算，包括 Pearson、Spearman 和 Kendall 相关系数。",style="color: #00aed1;line-height: 1.8;font-size: 18px;"), 
                   tags$li("回归分析，包括线性回归和逻辑回归。",style="color: #006aa2;line-height: 1.8;font-size: 18px;"),
                   #tags$li("生存分析，包括 Cox 回归分析。",style="color: #006aa2;")
                 ),
                 h4("请上传.csv格式数据，可用Excel导出CSV文件：",style="color: #ff8500;line-height: 1.8;font-size: 18px;"),
                 tags$a(href="案例.csv","数据格式案例（点击下载）",target="_blank;line-height: 1.8;font-size: 18px;"),
                 
                 h4("如有疑问，请联系平台管理人员：",style="color: #194b78;line-height: 1.8;font-size: 20px;"),
                 h5("孟栗典 Lidian Meng (Email: lidianmeng@chnu.edu.cn)。",style="color: #00aed1;line-height: 1.8;font-size: 18px;"),
                 br()
        ),
        
        tabPanel('使用说明',
                 
                 h4("实验研究型（目前较为主流的选题类型）",style="color: #194b78;line-height: 1.8;font-size: 20px;"),
                 
                 tags$ul(
                   tags$li("核心特征：设有实验组与对照组，通过训练或教学干预，采用前测/后测设计，运用统计方法检验干预效果是否显著。", style="color: #00aed1;line-height: 1.8;font-size: 18px;"),
                   tags$li("基本流程：研究对象 → 实验设计 → 实施干预 → 数据统计 → 效果分析", style="color: #006aa2;line-height: 1.8;font-size: 18px;"),
                   tags$li("常用方法：t检验、方差分析、效应量分析等。", style="color: #00aed1;line-height: 1.8;font-size: 18px;")
                 ),
                 
                 h4("问卷调查型（目前较为主流的选题类型）",style="color: #194b78;line-height: 1.8;font-size: 20px;"),
                 tags$ul(
                   tags$li("核心特征：编制或选用成熟问卷，进行大样本调查，对研究对象的现状及影响因素进行分析，并提出对策建议。", style="color: #00aed1;line-height: 1.8;font-size: 18px;"),
                   tags$li("现状调查 → 信效度检验 → 因素分析 → 回归分析 → 对策建议", style="color: #006aa2;line-height: 1.8;font-size: 18px;"),
                   tags$li("常用方法信度分析、效度检验、描述统计、因素分析、相关与回归分析、结构方程模型等。", style="color: #00aed1;line-height: 1.8;font-size: 18px;")
                 ),
                 
                 h4("文献计量与文献综述型研究（一般不单独作为学位论文）",style="color: #194b78;line-height: 1.8;line-height: 1.8;font-size: 20px;"),
                 tags$ul(
                   tags$li("核心特征：基于数据库文献，对某一研究领域的研究现状、热点与发展趋势进行系统梳理和计量分析。", style="color: #00aed1;line-height: 1.8;font-size: 18px;"),
                   tags$li("基本流程：文献检索 → 计量分析 → 研究热点 → 发展趋势 → 评价与展望", style="color: #006aa2;line-height: 1.8;font-size: 18px;"),
                   tags$li("常用方法文献计量分析、知识图谱分析等。", style="color: #00aed1;line-height: 1.8;font-size: 18px;")
                 ),
        ...
        tabPanel("2. 上传数据", 
                 ...
                 navbarPage(
                   title = '数据表选项',
                   tabPanel('数据',     DT::dataTableOutput('tab.data')),
                   tabPanel('汇总',verbatimTextOutput('summ.data')),
                   tabPanel('描述',htmlOutput('desc.data')),
                   tabPanel('R 数据列表', DT::dataTableOutput('rdata.data'))
                 )
        ),
        ...
        tabPanel('信度和效度分析',
                 # 只有选择了至少 2 个条目后才显示内容
                 conditionalPanel(
                   condition = "typeof input.reli_items !== 'undefined' && input.reli_items.length > 1",
                   
                   # 1. 条目描述
                   fluidRow(
                     column(10, offset = 1,
                            h2("量表条目描述"),
                            tableOutput("desc_reli")
                     )
                   ),
                   ...
                   fluidRow(
                     column(10, offset = 1,
                            h2("信度分析（Cronbach's α）"),
                            verbatimTextOutput("alpha_reli"),
                            h3("条目-总分相关（Item-total correlation）"),
                            tableOutput("itemtotal_reli")
                     )
                   ),
                   
                   # 3. 效度：总分 vs 效标变量的相关 + 散点图
                   conditionalPanel(
                     condition = "typeof input.validity_var !== 'undefined' && input.validity_var !== '' && input.validity_type === 'corr'",
        ...
        tabPanel('4. t检验',
                 conditionalPanel(condition = "(input.var1_ttest !== 'undefined' && input.var1_ttest.length > 0)",
                   ...
                   fluidRow(column(10, offset = 1,
                                   ...
                                   h2("效应量（Cohen's d）"),
                                   verbatimTextOutput("effect_ttest")
        ...
        tabPanel('双因素方差分析',
                 conditionalPanel(condition = "(input.var1_tanova !== 'undefined' && input.var1_tanova.length > 0 &&
                   ...
                   h2("效应量（partial η²）"),
                   verbatimTextOutput("peta2_tanova"),
        ...
        tabPanel('多重比较校正',
                 ...
                 h2("帮助"),
                 h6("Holm（1979）、Bonferroni、Hommel（1988）和 Hochberg（1988）方法用于严格控制整体第一类错误率（FWER，家族误差率）。由于 Holm 方法在任意假设条件下同样有效，并且在统计功效上优于传统 Bonferroni 校正，因此一般没有必要再使用未经改进的 Bonferroni 方法。", style="line-height: 1.8;font-size: 16px;"),
                 h6("当各假设检验之间相互独立或非负相关时，Hochberg 和 Hommel 方法是有效的。Hommel 方法的统计功效略高于 Hochberg 方法，但二者差别通常较小，而 Hochberg 方法计算速度更快。", style="line-height: 1.8;font-size: 16px;"),
                 h6("Benjamini–Hochberg（BH）和 Benjamini–Yekutieli（BY）方法用于控制假发现率（FDR），即在所有被拒绝的假设中错误拒绝的期望比例。FDR 的控制条件比 FWER 更宽松，因此 BH 和 BY 方法通常比其他方法具有更高的统计功效。", style="line-height: 1.8;font-size: 16px;")
        ),
        ...
        tabPanel('6. 版本信息',
                 ...
                 h4("V1.1，2025年12月5日",style="color: #194b78;line-height: 1.8;font-size: 20px;"),
                 
                 tags$ul(
                   tags$li("包含多种常用的统计方法", style="color: #00aed1;font-size: 18px;"),
                   
                 ),
                 
                 h4("V1.0，2025年12月1日",style="color: #194b78;line-height: 1.8;font-size: 20px;"),
                 
                 tags$ul(
                   tags$li("Meng Lab：体育学位论文数据在线分析平台项目成立", style="color: #00aed1;font-size: 18px;"),
                 ),
...
server = 
  function(input, output, session) {
    ...
    # 1）信度分析用的数据：用户在侧边栏选的条目
    reli_data <- reactive({
      req(data())             # 确保已经有数据
      req(input$reli_items)   # 确保选了条目
      
      data()[, input$reli_items, drop = FALSE]
    })
    
    # 2）条目描述：每个条目的均值、SD、NA 个数
    output$desc_reli <- renderTable({
      dat <- reli_data()
      
      data.frame(
        变量   = colnames(dat),
        均值   = sapply(dat, mean, na.rm = TRUE),
        标准差 = sapply(dat, sd,   na.rm = TRUE),
        NA个数 = sapply(dat, function(x) sum(is.na(x))),
        check.names = FALSE,
        row.names   = NULL
      )
    })
    ...
    # 3）信度：Cronbach's α
    output$alpha_reli <- renderPrint({
      dat <- reli_data()
      psych::alpha(dat)
    })
    
    # 4）条目-总分相关表（item-total）
    output$itemtotal_reli <- renderTable({
      dat <- reli_data()
      a <- psych::alpha(dat)
      
      # 从 psych::alpha 的结果里取条目信息
      res <- a$item.stats[, c("r.drop", "r.cor", "mean", "sd")]
      res$条目 <- rownames(res)
      res <- res[, c("条目", "mean", "sd", "r.drop", "r.cor")]
      
      colnames(res) <- c("条目", "均值", "SD", "纠正后条目-总分相关", "与总分相关")
      rownames(res) <- NULL
      res
    })
    ...
    total_score <- reactive({
      dat <- reli_data()
      # 这里用“各条目取平均”；你要改成加总也可以：rowSums(dat, na.rm = TRUE)
      rowMeans(dat, na.rm = TRUE)
    })
    
    # 6）效度：量表总分 vs 效标变量 的相关检验
    output$validity_cor <- renderPrint({
      req(data())
      req(input$validity_var)          # 用户选了效标变量
      req(input$validity_var != "")    # 不是空
      req(input$validity_type == "corr")
      
      y <- data()[[input$validity_var]]   # 效标变量
      x <- total_score()                  # 量表总分
      
      ok <- complete.cases(x, y)
      
      cor.test(x[ok], y[ok])              # 相关系数 + 显著性检验
    })
    
    # 7）效度：散点图
    output$validity_plot <- renderPlot({
      req(data())
      req(input$validity_var)
      req(input$validity_var != "")
      req(input$validity_type == "corr")
      
      y <- data()[[input$validity_var]]
      x <- total_score()
      ok <- complete.cases(x, y)
      
      plot(x[ok], y[ok],
           xlab = "量表总分",
           ylab = input$validity_var,
           main = "量表总分与效标变量的散点图")
      abline(lm(y[ok] ~ x[ok]))
    })
    ...
    # Updata value user could select  —— 信度和效度分析用的变量
    observe({
      req(data())
      # 这里我只把“数值型变量”作为量表条目和效标变量候选
    ...
    output$out.ttest <- renderPrint({
      ...
      output$effect_ttest <- renderPrint({
        # 非参数检验时不计算 Cohen's d
        if (input$nonparm_ttest == "yes") {
          cat("当前选择了非参数检验（Wilcoxon），本平台暂不计算 Cohen's d。")
        }
        
        # 置信：确保选择了变量
        req(data())
        req(input$sample_ttest)
        req(input$var1_ttest)
        ...
        if (input$sample_ttest == "oneSamp") {
          ...
          d <- (mean(new.var1, na.rm = TRUE) - input$null.ttest) / sd(new.var1, na.rm = TRUE)
          cat("Cohen's d =", round(d, 3))
        }
        
        # ---- 配对样本 ----
        if (input$sample_ttest == "pairSamp") {
          req(input$pair2.ttest)
          var1 <- data.frame(v1 = data()[, input$var1_ttest],
                             v2 = data()[, input$pair2.ttest])
          if (is.null(var1$v1))
          ...
          diffv <- new.var1$v1 - new.var1$v2
          d <- mean(diffv, na.rm = TRUE) / sd(diffv, na.rm = TRUE)
          cat("Cohen's d =", round(d, 3))
        }
        ...
        if (input$sample_ttest == "twoSamp") {
          ...
          if (input$isgroup_ttest == "no") {
            ...
            n1 <- length(new.var1); n2 <- length(new.var2)
            sd_pooled <- sqrt(((n1 - 1) * sd(new.var1)^2 + (n2 - 1) * sd(new.var2)^2) / (n1 + n2 - 2))
            d <- (mean(new.var1) - mean(new.var2)) / sd_pooled
            cat("Cohen's d =", round(d, 3))

          }
          
          # 分组：v ~ g
          if (input$isgroup_ttest == "yes") {
            req(input$group2.ttest)
            var1 <- data.frame(v = data()[, input$var1_ttest],
                               g = data()[, input$group2.ttest])
            if (is.null(var1$v)) 
            ...
            new.var1$g <- factor(new.var1$g)
            lev <- levels(new.var1$g)
            validate(need(length(lev) == 2, "分组变量必须恰好有两组，才能计算 Cohen's d。"))
            
            x1 <- new.var1$v[new.var1$g == lev[1]]
            x2 <- new.var1$v[new.var1$g == lev[2]]
            
            n1 <- length(x1); n2 <- length(x2)
            sd_pooled <- sqrt(((n1 - 1) * sd(x1)^2 + (n2 - 1) * sd(x2)^2) / (n1 + n2 - 2))
    ...
    output$out.anova <- renderPrint({
      output$eta2_anova <- renderPrint({
        var1 <- na.omit(data.frame(v = data()[, input$var1_anova],
                                   g = data()[, input$var2_anova]))
        if (is.null(var1$v)) return(NULL)
        ...
        if (input$transform.anova == "none") {
          ...
          wdata <- var1
          wdata$v <- sqrt(wdata$v)
        }
        
        wdata$g <- factor(wdata$g)
        
        # 非参数：先不算 η²（如需我也可以给你加 epsilon²）
        if (input$nonparm_anova == "yes") {
          cat("当前为非参数检验（Kruskal-Wallis），本平台暂不计算 η²。")
          invisible()
          return(NULL)
        }
        
        fit <- aov(v ~ g, data = wdata)
        tab <- summary(fit)[[1]]
        ss_effect <- tab[1, "Sum Sq"]
        ss_error  <- tab[nrow(tab), "Sum Sq"]
    ...
    output$out.tanova <- renderPrint({
      output$peta2_tanova <- renderPrint({
        var1 <- data()[, c(input$var1_tanova, input$var2_tanova, input$var3_tanova)]
        
        # 跟 out.tanova 同步的变换逻辑
        if (input$transform.tanova == "none") {
          tpos <- which(!is.na(var1[, input$var1_tanova]))
          new.var1 <- var1[tpos, ]
        } else if (input$transform.tanova == "logtrans") {
        ...
        for (i in seq_along(terms)) {
          ss <- tab[terms[i], "Sum Sq"]
          out$partial_eta2[i] <- ss / (ss + ss_err)
        }
        
        out$partial_eta2 <- round(out$partial_eta2, 3)
        out
      })
